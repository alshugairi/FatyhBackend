...
@section('content')
    <div class="app-content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="page-title">{{ __('admin.attributes') }}</h3>
                </div>
                <div class="col-sm-6 text-end">
                    ...
                    @can('admins.create')
                        <x-datatable.create href="{{ route('admin.attributes.create') }}" module="{{ __('admin.attribute') }}"/>
                    @endcan
                </div>
            </div>
        </div>
                ...
                columnDefs: [
                    @canany(['attributes.edit','attributes.delete'])
                    {
                        ...
                        "render": function (data, type, row) {
                            var editUrl = '{{ route("admin.attributes.edit", ":id") }}';
                            editUrl = editUrl.replace(':id', row.id);

                            var deleteUrl = '{{ route("admin.attributes.destroy", ":id") }}';
                            deleteUrl = deleteUrl.replace(':id', row.id);

                            var optionsUrl = '{{ route("admin.attributes.options.index", ":id") }}';
                            optionsUrl = optionsUrl.replace(':id', row.id);

                            return `<div class='btn-actions'>
                                       @can('attributes.edit')
                                       <a class="text-info" href="${optionsUrl}"><i class="fa-solid fa-eye mx-1"></i></a>
                                       @endcan
                                       @can('attributes.edit')
