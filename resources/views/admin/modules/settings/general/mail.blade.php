                    ...
                    <form action="{{ route('admin.settings.mail.store') }}" method="post" enctype="multipart/form-data" id="submitForm">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">{{ __('admin.mail') }}</h3>
                                <div class="card-tools">
                                    <x-datatable.save module="{{ __('admin.mail') }}"/>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <x-form.select col="col-md-6" label="true" name="mail_encryption" required="true" :options="$encryptionTypes" labelName="{{ trans('admin.encryption') }}" value="{{ $settings->where('key', 'mail_encryption')->first()?->value }}"/>
                                </div>
                            </div>
                        </div>
                    </form>
