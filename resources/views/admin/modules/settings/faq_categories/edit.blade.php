        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-9 col-md-6 col-sm-12">
                    <form class="validate-form" action="{{ route('admin.faq_categories.update', $faqCategory->id) }}" method="post" id="submitForm">
                        @method('put')
                        @csrf
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">{{ __('admin.edit') }} {{ __('admin.faq_category') }}</h3>
                                <div class="card-tools">
                                    <x-datatable.save module="{{ __('admin.faq_category') }}"/>
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    @foreach($appLanguages as $appLanguage)
                                        <div class="tab-pane fade{{ $loop->first ? ' show active' : '' }}" id="navs-top-{{ $appLanguage->id }}" role="tabpanel">
                                            <x-form.input type="text" name="name[{{ $appLanguage->code }}]" required="true"
                                                          ...
                                                          value="{!! $faqCategory->getTranslation('name', $appLanguage->code) !!}"
                                            ...
                                            <x-form.textarea type="text" name="description[{{ $appLanguage->code }}]"
                                                             ...
                                                             value="{!! $faqCategory->getTranslation('description', $appLanguage->code) !!}"
                                                             label="true" labelName="{{ __('admin.description') }}"/>
                                        </div>
                                    @endforeach
                                </x-layouts.language-tabs>
                                <div class="row">
                                    <x-form.input col="col-md-4" required="true" type="number" name="order" value="{{ $faqCategory->order }}" labelName="{{ trans('admin.order') }}"/>
                                    <x-form.switch col="col-md-4" name="is_active" value="{{ $faqCategory->is_active }}" labelName="{{ trans('admin.active') }}"/>
                                </div>
                            </div>
                        </div>
                    </form>
